import { BrowserWindow, dialog, app, shell, ipcMain } from "electron";
import path, { join, resolve, basename, dirname } from "node:path";
...
import { platform as platform$1, homedir, arch } from "os";
import { exec as exec$1, spawn, execFile, spawnSync } from "node:child_process";
...
async function detectInstallationType(pythonPath) {
  ...
  try {
    ...
    const { stdout } = await execAsync$6(`"${pythonPath}" -c "import sys; print(sys.prefix)"`);
    const normalizedStdout = normalize(stdout);
    if (normalizedStdout.includes("conda")) return "conda";
    const { stdout: versionStdout } = await execAsync$6(`"${pythonPath}" --version`);
...
async function parseVersion(pythonPath) {
  return new Promise(async (resolve2, reject) => {
    try {
      const { stdout } = await execAsync$6(`"${pythonPath}" --version 2>&1`);
      const versionMatch = stdout.trim().match(/Python (\d+)\.(\d+)(?:\.(\d+))?/i);
      if (lodashExports.isNil(versionMatch)) {
...
async function openDialogExt(options2) {
  try {
    ...
    const result = await (mainWindow ? dialog.showOpenDialog(mainWindow, options2) : dialog.showOpenDialog(options2));
...
async function getPythonVersion(venvPath) {
  return new Promise((resolve2, reject) => {
    ...
    exec$1(`"${pythonExecutable}" --version`, (error, stdout, stderr) => {
      if (error) {
        reject(error);
        return;
      }
...
async function getSitePackagesUpdates(pythonExePath) {
  return new Promise((resolve2, reject) => {
...
async function locateAIVenv(id) {
  try {
...
function updateAIVenvStorage(data) {
  const existingData = storageManager?.getCustomData(AI_VENV_STORE_KEYS);
...
function getAIVenv(id) {
  const data = storageManager?.getCustomData(AI_VENV_STORE_KEYS);
  return data?.find((item) => item.id === id)?.path;
}
function getAIVenvs() {
  return storageManager?.getCustomData(AI_VENV_STORE_KEYS);
}
function addAIVenv(id, pythonPath) {
  updateAIVenvStorage({ id, path: pythonPath });
  checkAIVenvsEnabled();
}
function removeAIVenv(id) {
  const existingData = storageManager?.getCustomData(AI_VENV_STORE_KEYS);
  if (existingData) {
...
function removeAIVenvPath(pythonPath) {
  const existingData = storageManager?.getCustomData(AI_VENV_STORE_KEYS);
  if (existingData) {
...
async function checkArguments(id, pythonExe) {
  ...
  if (!data) return;
  function findOrUpdateArgument(name, value) {
...
async function removeArguments(id, pythonExe) {
  const venvPath = dirname(dirname(pythonExe));
  const existingArgs = await storageManager?.getCardArgumentsById(id);
  if (!existingArgs) return;
  const activePreset = existingArgs.activePreset;
  ...
  function removeArgumentIfPathMatches(name, pathToRemove) {
...
async function checkAIVenvsEnabled() {
  const venvs = getAIVenvs();
  if (lodashExports.isNil(venvs) || lodashExports.isEmpty(venvs)) {
    return;
  }
  const updatePromises = [];
  const preCommandUpdates = [];
...
async function removeAIVenvsEnabled(id, pythonPath) {
  const argumentsPromises = [];
  const preCommands = [];
  switch (id) {
    case "LSHQQYTIGER_SD":
    case "LSHQQYTIGER_Forge_SD":
...
function requireLib() {
  ...
  const { join: join2, delimiter, sep, posix: posix2 } = require$$1$1;
...
async function uninstallCondaPython(pythonPath) {
  try {
    const envName = await getCondaEnvName(pythonPath);
    await execAsync$4(`conda env remove -n ${envName} -y`);
    return {
      success: true,
  ...
  try {
    ...
    for (const cmd of pythonCommands) {
      try {
        ...
        if (path2) paths.push(path2.replace(".EXE", ".exe"));
      } catch (error) {
      }
    }
    return paths;
...
async function findInCommonLocations() {
  ...
  for (const pathPattern of paths) {
    const expandedPath = pathPattern.replace("%LOCALAPPDATA%", process.env.LOCALAPPDATA || "").replace("%USERPROFILE%", homedir()).replace("~", homedir());
...
async function getSitePackagesPath(pythonPath) {
  try {
    const { stdout } = await execAsync$3(`"${pythonPath}" -c "import site; print(site.getsitepackages()[1])"`);
    return stdout.trim();
  } catch (error) {
    throw new Error(`Failed to get site-packages location: ${error}`);
  }
}
async function detectPythonInstallations() {
  const installations = [];
...
async function listAvailablePythons() {
  return new Promise((resolve2, reject) => {
    exec$1("conda search python --info", (error, stdout, stderr) => {
      if (error) {
      ...
      resolve2([...new Set(versions)]);
    });
  });
}
const createCondaEnv = async (envName, pythonVersion) => {
  return new Promise((resolve2, reject) => {
    const window2 = BrowserWindow.getFocusedWindow();
    const command = `conda create --name ${envName} python=${pythonVersion} -y`;
    const process2 = spawn(command, { shell: true });
...
function isCondaInstalled() {
  return new Promise((resolve2) => {
    exec$1("conda --version", (error, _stdout, stderr) => {
      if (error || stderr) {
        resolve2(false);
      } else {
        resolve2(true);
...
function registerListener(session, options2, callback = () => {
  ...
  const listener = (event, item, webContents) => {
    ...
    const directory = options2.directory ?? app.getPath("downloads");
    let filePath;
...
async function installOnWindows(installerPath) {
  return new Promise((resolve2, reject) => {
    execAsync$2(`"${installerPath}" /quiet InstallAllUsers=0 PrependPath=1 Include_test=0 Include_pip=1 `).then(() => resolve2()).catch(() => {
      execAsync$2(`"${installerPath}" /repair`).then(() => resolve2()).catch((e) => {
        reject(e);
      });
    });
  });
...
function findValidRequirementsFiles(dirPath) {
  try {
    ...
    for (const file of files) {
      if (file.includes("requirements") && gracefulFsExports.statSync(join$1(dirPath, file)).isFile()) {
...
function setReqPath(data) {
  const existingData = storageManager?.getCustomData(REQ_STORE_ID);
  let result = [];
  if (existingData) {
...
function getReqPath(id) {
  const data = storageManager?.getCustomData(REQ_STORE_ID);
  return data?.find((item) => item.id === id)?.path;
}
const execAsync$1 = promisify$1(exec$1);
async function getLatestVersion(pythonPath, name) {
  let latestVersion;
  try {
    const latestVersionResult = await execAsync$1(
...
async function checkPackageUpdates(pythonPath, reqPath, currentPackages) {
  const result = [];
...
const defaultPackageCachePath = join(homedir$1(), "AppData", "Local", "Package Cache");
...
async function uninstallPythonWindows(installerPath) {
  return new Promise((resolve2, reject) => {
    ...
    execFile(installerPath, uninstallArgs, (error, stdout) => {
      ...
      resolve2("Python uninstalled successfully");
    });
  });
}
...
async function uninstallPython(pythonPath) {
  try {
...
function createPythonVenv(options2) {
...
async function initialExtension(lynxApi, utils) {
  ...
  lynxApi.listenForChannels(() => {
    ...
    ipcMain.handle(pythonChannels.uninstallPython, (_, path2) => uninstallPython(path2));
    ipcMain.handle(pythonChannels.setDefaultPython, (_, pythonPath) => setDefaultPython(pythonPath));
    ipcMain.handle(pythonChannels.getAvailableOfficial, () => getAvailablePythonVersions());
    ipcMain.handle(pythonChannels.installOfficial, (_, version) => downloadPython(version));
    ipcMain.handle(pythonChannels.getAvailableConda, () => listAvailablePythons());
    ipcMain.handle(
      pythonChannels.installConda,
      (_, envName, version) => createCondaEnv(envName, version)
    );
    ipcMain.handle(pythonChannels.isCondaInstalled, () => isCondaInstalled());
    ipcMain.handle(pythonChannels.createVenv, (_, options2) => createPythonVenv(options2));
    ipcMain.handle(pythonChannels.getVenvs, () => getVenvs());
    ipcMain.handle(pythonChannels.locateVenv, () => locateVenv());
    ipcMain.handle(pythonChannels.getPackagesInfo, (_, pythonPath) => getSitePackagesInfo(pythonPath));
    ipcMain.handle(
    ...
    ipcMain.handle(pythonChannels.getPackagesUpdateInfo, (_, pythonPath) => getSitePackagesUpdates(pythonPath));
    ipcMain.handle(
    ...
    ipcMain.handle(pythonChannels.readReqs, (_, filePath) => readRequirements(filePath));
    ipcMain.handle(pythonChannels.saveReqs, (_, filePath, data) => saveRequirements(filePath, data));
    ipcMain.handle(pythonChannels.findReq, (_, dirPath) => findValidRequirementsFiles(dirPath));
    ipcMain.on(pythonChannels.setReqPath, (_, data) => setReqPath(data));
    ipcMain.handle(pythonChannels.getReqPath, (_, id) => getReqPath(id));
    ipcMain.handle(pythonChannels.locateAIVenv, (_, id) => locateAIVenv(id));
    ipcMain.handle(pythonChannels.getAIVenv, (_, id) => getAIVenv(id));
    ipcMain.on(pythonChannels.addAIVenv, (_, id, pythonPath) => addAIVenv(id, pythonPath));
    ipcMain.on(pythonChannels.removeAIVenv, (_, id) => removeAIVenv(id));
    ipcMain.on(pythonChannels.removeAIVenvPath, (_, path2) => removeAIVenvPath(path2));
    ipcMain.handle(pythonChannels.getAIVenvs, () => getAIVenvs());
    ipcMain.handle(pythonChannels.findAIVenv, (_, id, folder) => findAIVenv(id, folder));
    ipcMain.on(pythonChannels.checkAIVenvEnabled, () => checkAIVenvsEnabled());
    ipcMain.handle(
      pythonChannels.getUpdatesReq,
      (_, pythonPath, reqFile, currentPackages) => checkPackageUpdates(pythonPath, reqFile, currentPackages)
    );
  });
}
export {
